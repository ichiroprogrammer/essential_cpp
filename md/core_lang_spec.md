...
constexpr変数がif文のオカレンスになる場合、[constexpr if文](---)することで、
...
このドキュメントでは慣用的に[constexpr変数](---)と呼んでいる概念が、コア定数式である。
...
### constexprインスタンス
[constexpr変数](---)を引数にして、[リテラル型](---)のconstexprコンストラクタを呼び出せば、
...
```cpp
    // @@@ example/core_lang_spec/template_ut.cpp #4:0 begin -1
```
...
C++では、[try-catch](---)構文や[エクセプションのthrow](---)文使用してエクセプション処理を実現する。
...
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #0:0 begin -1
```
...
エクセプションのtry-catchは[関数tryブロック](---)によっても実装できる。

一般にエクセプションオブジェクトのcatchには上記ののコード例のように`T const&`を使用することが好ましい。
これにより、エクセプションオブジェクトの[オブジェクトのコピー|スライシング](---)を避けることができる。

### 関数tryブロック
関数tryブロックとはtry-catchを本体とした下記のような関数のブロックを指す。

```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #1:0 begin
```

### エクセプションのthrow
throwキーワードを使用して、任意の型のオブジェクトをエクセプションとして投げる(throw)ことができるが、
throwのオペランドには、慣習的に[std::exception](---)の派生クラスのインスタンスを用いることが多い。

```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #2:0 begin
```
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #2:1 begin -1
```
...
`throw e;`では、元のエクセプションオブジェクトが[オブジェクトのコピー|スライシング](---)される可能性があるためである。
...
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #2:2 begin -1
```
...
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #3:0 begin
```
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #3:1 begin -1
```

演算子としてのnoexceptはテンプレートで頻繁に使用されるため、以下にそのような例を示す。

```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #3:2 begin
```
```cpp
    // @@@ example/core_lang_spec/exception_ut.cpp #3:3 begin -1
```
