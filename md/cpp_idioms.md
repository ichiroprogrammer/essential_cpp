...
それと一体で判断すべき依存構造の原則 [ADP(非循環依存の原則)](---)を用いる。

| 原則                           | 一文での定義                             | 視点   | 力の向き／作用   |
|:------------------------------ |:-----------------------------------------|:-------|:-----------------|
| [REP(リリース等価の原則)](---) | リリースノートが一本の筋として書けるか   | 提供側 | 凝集（まとめる） |
| [CCP(共通閉鎖の原則)](---)     | 変更される理由が一つか                   | 提供側 | 凝集（まとめる） |
| [CRP(共通再利用の原則)](---)   | 利用者に不要な依存まで抱えさせていないか | 利用側 | 分割（割る）     |
...
CCPは、[REP(リリース等価の原則)](---)と同様に「パッケージをまとめることの根拠」になり得る。
...
[REP(リリース等価の原則)](---)/[CCP(共通閉鎖の原則)](---)とは逆に、「パッケージを分割することの根拠」になり得る。
...
- パッケージのソースコードツリーは、[Modern CMake project layout](---)でなければならない。
