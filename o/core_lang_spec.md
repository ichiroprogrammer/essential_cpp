...
ポリモーフィックなクラスは、
仮想関数呼び出しを行う(「[オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_4_8_1)」参照)
ためのメモリレイアウトが必要になる。
...
各クラスがvtblへのポインタを保持するため、このドキュメントで使用している[g++](cpp_idioms.md#SS_4_10_1)では、
...
このドキュメントのコードのコンパイルに使用している[g++](cpp_idioms.md#SS_4_10_1)/[clang++](cpp_idioms.md#SS_4_10_2)では、
...
これにより、[Static Initialization Order Fiasco(静的初期化順序問題)](cpp_idioms.md#SS_4_9_11)を回避できる。
...
* [ゼロの原則(Rule of Zero)](cpp_idioms.md#SS_4_5_1)
* [五の原則(Rule of Five)](cpp_idioms.md#SS_4_5_2)

この2つの原則(ガイドライン)の使い分けに関しては、

* リソース管理を外部([RAII(scoped guard)](design_pattern.md#SS_6_10)クラス)に任せられる場合: ゼロの法則を採用し、特殊メンバ関数を明示的に定義しない。
...
といった[セマンティクス](cpp_idioms.md#SS_4_11_1)的観点によるものである。
...
このような状態のリファレンスを[danglingリファレンス](cpp_idioms.md#SS_4_8_2)と呼ぶ。
同様に、このような状態のポインタを[danglingポインタ](cpp_idioms.md#SS_4_8_3)と呼ぶ。
...
* C++11の[moveセマンティクス](cpp_idioms.md#SS_4_3_3)と[perfect forwarding](core_lang_spec.md#SS_2_8_5)を実現するために導入された。
...
この動作を理解することは、
[moveセマンティクス](cpp_idioms.md#SS_4_3_3)や[perfect forwarding](core_lang_spec.md#SS_2_8_5)を正しく実装/使用するために極めて重要である。
...
下記GetString0()のような関数が返すオブジェクトの内部メンバに対する[ハンドル](cpp_idioms.md#SS_4_9_6)は、
...
サンプルコードを[g++](cpp_idioms.md#SS_4_10_1)と[clang++](cpp_idioms.md#SS_4_10_2)でコンパイルしている)。
...
によるとこの動作は未定義のようだが、
[g++](cpp_idioms.md#SS_4_10_1)/[clang++](cpp_idioms.md#SS_4_10_2)両方ともこのコードを警告なしでコンパイルする)。
...
このテストはエラーとならなければならないが、[g++](cpp_idioms.md#SS_4_10_1)ではパスしてしまう。
2nd name lookupのロジックにバグがあるようである。

有難いことに、[clang++](cpp_idioms.md#SS_4_10_2)では仕様通りこのテストはエラーとなり、
