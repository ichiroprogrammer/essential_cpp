...
ような場合に使用する。
そのため[オープン・クローズドの原則(OCP)](solid.md#SS_5_2)の実装方法としても有用である。
...
bird::flyのオーバーライド関数(penguin::fly)について、[リスコフの置換原則(LSP)](solid.md#SS_5_3)に反した例を下記する。
...
下記に示す通り、このコードは、
[オープン・クローズドの原則(OCP)](solid.md#SS_5_2)にも対応した柔軟な構造を実現している。
...
* `LCOM <= 0.4`  
  クラスが非常に凝集しており、[単一責任の原則(SRP)](solid.md#SS_5_1)を強く遵守している状態であるため、
  通常、デザインの見直しは不要である。
...
* 「[単一責任の原則(SRP)](solid.md#SS_5_1)」を守ると凝集性は高くなりやすい。
