...
ポリモーフィックなクラスは、
仮想関数呼び出しを行う(「[オーバーライドとオーバーロードの違い](cpp_idioms.md#SS_4_7_1)」参照)
ためのメモリレイアウトが必要になる。
...
各クラスがvtblへのポインタを保持するため、このドキュメントで使用している[g++](cpp_idioms.md#SS_4_9_1)では、
...
このドキュメントのコードのコンパイルに使用している[g++](cpp_idioms.md#SS_4_9_1)/[clang++](cpp_idioms.md#SS_4_9_2)では、
...
これにより、[Static Initialization Order Fiasco(静的初期化順序問題)](cpp_idioms.md#SS_4_8_8)を回避できる。
...
* [ゼロの原則(Rule of Zero)](cpp_idioms.md#SS_4_4_1)
* [五の原則(Rule of Five)](cpp_idioms.md#SS_4_4_2)
...
といった[セマンティクス](cpp_idioms.md#SS_4_10_1)的観点によるものである。
...
このような状態のリファレンスを[danglingリファレンス](cpp_idioms.md#SS_4_7_2)と呼ぶ。
同様に、このような状態のポインタを[danglingポインタ](cpp_idioms.md#SS_4_7_3)と呼ぶ。
...
下記GetString0()のような関数が返すオブジェクトの内部メンバに対する[ハンドル](cpp_idioms.md#SS_4_8_3)は、
...
サンプルコードを[g++](cpp_idioms.md#SS_4_9_1)と[clang++](cpp_idioms.md#SS_4_9_2)でコンパイルしている)。
...
によるとこの動作は未定義のようだが、
[g++](cpp_idioms.md#SS_4_9_1)/[clang++](cpp_idioms.md#SS_4_9_2)両方ともこのコードを警告なしでコンパイルする)。
...
このテストはエラーとならなければならないが、[g++](cpp_idioms.md#SS_4_9_1)ではパスしてしまう。
2nd name lookupのロジックにバグがあるようである。

有難いことに、[clang++](cpp_idioms.md#SS_4_9_2)では仕様通りこのテストはエラーとなり、
