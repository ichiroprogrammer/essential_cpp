...
最も単純な使用例を以下に示す(「[Spurious Wakeup](cpp_idioms.md#SS_4_12_15)」参照)。
```cpp
...
一般に条件変数には、[Spurious Wakeup](cpp_idioms.md#SS_4_12_15)という問題があり、std::condition_variableも同様である。

上記の抜粋である下記のコード例では[Spurious Wakeup](cpp_idioms.md#SS_4_12_15)の対策が行われていないため、
意図通り動作しない可能性がある。
